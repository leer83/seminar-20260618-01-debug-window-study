...
/**
 * @update 2022-12-07 09:34:56 - JGY :영역제한에 factor값 계산 추가
 * @update 2022-12-07 09:34:56 - JGY :collide옵션을 추가
 */
...
$.widget("ui.draggable", $.ui.mouse, {
	...
	_setContainment: function() {
		...
		// 2022-12-07 09:34:56 - JGY
		// 영역 제한
		this.containment.forEach(function (value, index, array) {
...
//    }
//
//    if (!isMobile) factor = parent.ZOOMVALUE;
	//getScale();
    var factor = 1;
	...
	return function( draggable, droppable, toleranceMode, event ) {
		...
		var x1 = (( draggable.positionAbs || draggable.position.absolute ).left / factor) + draggable.margins.left,
			y1 = (( draggable.positionAbs || draggable.position.absolute ).top / factor) + draggable.margins.top,
			x2 = x1 + draggable.helperProportions.width,
			y2 = y1 + draggable.helperProportions.height,
			l = droppable.offset.left / factor,
			t = droppable.offset.top / factor,
			r = l + droppable.proportions().width,
			b = t + droppable.proportions().height;

		switch (toleranceMode) {

			// 2022-12-07 09:41:11 - JGY
			// 'collide(충돌)'를 추가
			case 'custom':
				/*
		...
		case "pointer":
			return isOverAxis( event.pageY/factor, t, droppable.proportions().height ) && isOverAxis( event.pageX/factor, l, droppable.proportions().width );
		case "touch":
			return (
